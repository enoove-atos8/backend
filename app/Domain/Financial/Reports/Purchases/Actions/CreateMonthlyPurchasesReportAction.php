...
class CreateMonthlyPurchasesReportAction
{
     ...
     */
    public function execute(MonthlyPurchasesReportData $monthlyPurchasesReportData): void
    {
        $cardsByTenant = $this->getCardsAction->execute();

        if (count($cardsByTenant) > 0) {
            foreach ($cardsByTenant as $card) {
                $monthlyPurchasesReportData->cardId = $card->id;
                $report = $this->monthlyPurchasesReportsRepository->generateMonthlyPurchasesReport($monthlyPurchasesReportData);
